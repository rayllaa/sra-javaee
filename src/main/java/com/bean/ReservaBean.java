...
public class ReservaBean implements Serializable{
	...
	public void reserva() {
		...
		for(int i  = 0; i < refeicoes.size(); i++) {
			...
			if(r.getAlimentosReservados().size() == 0) { //aluno deu check na refeicao mas não especificou alimento, salva todos alimentos da refeicao
	...
	public List<ReservaVO> quantidadeReservaRefeicao() { 
		List<ReservaVO> reservas_ref = new ArrayList<ReservaVO>();
		List<RefeicaoVO> refeicoes = dao_refeicao.getRefeicoesData(); 
		
		for(int i = 0; i < refeicoes.size(); i++) {
			int reservas = 0;
			RefeicaoVO r = refeicoes.get(i);
			ReservaVO res = new ReservaVO();
			reservas = dao_reserva.reservaRefeicao(r.getData());
	...
	public List<ReservaVO> reservaAlunos() {
